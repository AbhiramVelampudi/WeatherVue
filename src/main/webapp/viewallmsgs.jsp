...
<th>ACTION</th>
						...
						<tbody>
							 <c:forEach items="${formdata}"  var="form"> 
<tr> 
<td><c:out value="${form.id}" /></td> 
<td><c:out value="${form.name}" /></td> 
<td><c:out value="${form.email}" /></td> 
<td><c:out value="${form.subject}" /></td> 
<td><c:out value="${form.message}" /></td> 
<td>
<a href='<c:url value="viewmsg?id=${form.id}"></c:url>'>View</a>
</td>
...
function downloadCSV() {
        updateFilteredData();
        var csv = 'ID,NAME,EMAIL ID,SUBJECT,MESSAGE\n';
        for (var i = 0; i < filteredData.length; i++) {
            // Combine all elements except the last one with a comma
            var row = filteredData[i].slice(0, 4).join(',');
            // Enclose the message within quotes to prevent splitting
            var message = filteredData[i][4];
            row += ',"' + message + '"\n';
            csv += row;
        }
